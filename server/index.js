...
const ABSOLUTE_UPLOAD_DIR =  __dirname + "/uploads/"  // 저장될 파일의 서버상 절대 경로
const ABSOLUTE_DOWNLOAD_DIR = __dirname + "/downloads/" // 클라이언트에서 다운받을 파일의 서버상 절대 경로

const RELATIVE_UPLOAD_DIR = "/project/uploads/"  // 클라이언트가 upload한 파일을 요청할 경로
const RELATIVE_DOWNLOAD_DIR = "/project/downloads/"  // 클라이언트가 download할 파일을 요청할 경로
...
/* 동영상 업로드  */
app.post("/project/upload", upload.single("selectFile"), (req, res) => {
...
/* 모자이크 Go! */
app.post("/project/mosaic", async (req, res) => {
    const result = new Response();
    ...
    // 방법 1
    let options  = {
        mode : "text",  // mode는 text와 json이 존재
        pythonOptions : ['-u'], // get print results in real-time
        args : option_list
        //pythonPath : "",    // pythonPath와 scriptPath는 각각 파이썬이 설치된 경로 해당 스크립트 파일이 설치된 경로인데, 환경변수로 지정되어 있으면 값이 없어도 정상적으로 실행이 됨. 
        //scriptPath : "",
    };
    ...
    try{
        await new Promise((resolve, reject) => {
            PythonShell.run("testing.py", options, function(err, results){
                ...
                console.log(`results : ${results}`) // results is an array consisting of messages collected during execution
            })
        })
    }catch(err){
        // 모자이크 처리 실패 // TODO: error 처리
        result.error = true
        console.log(err)
    }
...
/* 동영상 다운로드 */
app.post("/project/download", async (req, res) => {
